...
CREATE OR REPLACE FUNCTION public.complete_appointment(_appointment_id uuid, _payment_method text DEFAULT NULL::text)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  v_method := CASE
    ...
    WHEN _payment_method IN ('pix','pix_qr','card','cash') THEN _payment_method
    WHEN v_appt.payment_method IN ('pix','pix_qr','card','cash') THEN v_appt.payment_method
    ELSE 'cash' END;
