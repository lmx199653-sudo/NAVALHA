-- Agendamentos presos aguardando pagamento online voltam para pagamento no local.
UPDATE public.appointments
   SET payment_state = 'pending', payment_expires_at = NULL, payment_method = 'cash_or_on_site'
 WHERE payment_state = 'payment_pending';

-- book_appointment sem dependência de expiração por pagamento.
...
-- complete_appointment sem taxa de uso da plataforma.
...
-- Funções exclusivas da integração.
DROP FUNCTION IF EXISTS public.mercado_pago_status(uuid);
DROP FUNCTION IF EXISTS public.public_payment_enabled(uuid);
DROP FUNCTION IF EXISTS public.appointment_payment_status(uuid);
DROP FUNCTION IF EXISTS public.barbershop_billing_summary(uuid);
DROP FUNCTION IF EXISTS public.expire_pending_payment_appointments(uuid);

-- Tabelas exclusivas da integração (sem dados de clientes/agendamentos).
DROP TABLE IF EXISTS public.usage_fees;
...
DROP TABLE IF EXISTS public.mercado_pago_webhook_events;
DROP TABLE IF EXISTS public.mercado_pago_accounts;

DROP TYPE IF EXISTS public.usage_fee_status;
DROP TYPE IF EXISTS public.platform_subscription_status;
DROP TYPE IF EXISTS public.payment_type;
DROP TYPE IF EXISTS public.payment_state;